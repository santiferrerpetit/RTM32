...
def build_tests() -> List[TestCase]:
    ...
    # JALR: codificado con rd=31; el manual describe R[31]=PC+4.
    # En el PDF actualizado $31 es $at (assembler temporary), no $ra.
    jalr_addr = addr(n)
    add("JALR — salto con enlace a $31/$at (rd=31)",
        [("reg", 2, 0x104)],
    ...
    add("ADDI — suma inmediata",
        [("reg", 2, 0)],
        addr(n), encode_i(0b00001, 2, 1, 5), "ADDI $1, $2, 5",
        {"r1": 5}, cause=0,
        notes="Funciona correctamente con reset previo. El bug original era un falso positivo por CPU trabada.")
